...
基础骨架里程碑已经完成测试、文档和验收。当前增量只实现基础 RAG 的离线半链路：

- 基于 Thrift/Hertz 的 Markdown `multipart/form-data` 上传与异步任务查询；
- Markdown 解析、确定性过滤、规范化和结构感知切片；
- 智谱 `embedding-3`、1024 维向量生成；
- Ent 映射、pgvector 持久化和仓库外受控执行的 DDL；
- 默认 1 worker、8 个等待任务的进程内有界队列；
- 相同 `document_key` 去重，以及新版本成功后切换并删除旧版本。

本增量不得提前加入在线检索、重排、事实校验、SSE 对话与商品卡片、购物车、订单或多模态逻辑。离线导入使用普通 Go Pipeline，不使用 Eino Graph。

## 阶段门禁

基础骨架已经验收。后续能力仍按以下门禁交付，当前离线半链路完成测试、文档和验收后才能继续基础 RAG 的在线部分：

1. 基础 RAG 在线部分：精确向量检索、重排、事实校验、SSE 与商品卡片；有性能证据后才评估 HNSW。
